...
QVector<CSnapshot*> snapshots;
double selectedTimeStep = 0;
QVector<int> times;
double drawdownRange[2];
...
public:
    SpectrogramData()
    {
        setInterval( Qt::XAxis, QwtInterval( 0, snapshots[selectedTimeStep]->getNumberOfNodes(1) ) );
        setInterval( Qt::YAxis, QwtInterval( 0, snapshots[selectedTimeStep]->getNumberOfNodes(0) ) );
        setInterval( Qt::ZAxis, QwtInterval( (int)drawdownRange[0],(int)drawdownRange[1]));
    }

    virtual double value( double x, double y ) const
    {
        int i = (int)x + (int)y * snapshots[selectedTimeStep]->getNumberOfNodes(1);
        return snapshots[selectedTimeStep]->getDrawdown(i);
    }
};
    ...
    d_alpha(255)
{
    ...
    for ( double level = 0.5; level < drawdownRange[1]; level += 1.0 )
        contourLevels += level;
...
void Plot::setTimeStep( int number )
{
...
void Plot::printPlot()
{
    ...
    printer.setOutputFileName( "conePlusPlus.pdf" );
